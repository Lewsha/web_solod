...
async function patch(url_root: string, id: number, obj: Object) {
  const url = `${url_root}?id=${id}`;
  return await fetch(url, {
    method: 'patch',
    body: JSON.stringify(obj)
  })
...
function build_url(url_root: string, sort: string, sort_field: string, filter: string, filter_field: string) {
  ...
  if (sort || filter) {
    ...
    if (filter) url += `filter=${filter}&filterField=${filter_field}`;
  }
  return url;
}
...
export default {
  ...
  patch_requested_payment: async (id: number, obj: Payment) => {
    return await patch("/payment/requested", id, obj)
  },
  ...
  patch_payment: async (id: number, obj: Payment) => {
